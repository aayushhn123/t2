...
def determine_default_slot(semester):
    sem = int(str(semester).strip())
    pos = (sem + 1)//2 if sem % 2 != 0 else sem//2
    return "10:00 AM - 1:00 PM" if pos % 2 == 1 else "2:00 PM - 5:00 PM"
...
def find_next_valid_day(start_day, holidays):
    d = start_day
    while d.weekday() == 6 or d in holidays:
        d += timedelta(days=1)
    return d

def schedule_electives(df, holidays, start_after):
    df = df.copy()
            ...
            with st.spinner("Processing your timetable... Please wait..."):
                try:
                    ...
                    if df_non_elec is not None and df_ele is not None:
                        ...
                        df_all = pd.concat([df_non_elec, df_ele], ignore_index=True)
                        df_sched = schedule_streamwise(df_all, holidays_set, base_date)
                        last_non_elec = pd.to_datetime(df_sched[df_sched['OE'].isna()]['Exam Date'], format="%d-%m-%Y", errors='coerce').max().date()
                        df_final = schedule_electives(df_sched, holidays_set, last_non_elec if pd.notna(last_non_elec) else base_date)

                        final_df = df_final.sort_values(["Semester", "MainBranch"], ascending=True, na_position='last')
                        sem_dict = {}
